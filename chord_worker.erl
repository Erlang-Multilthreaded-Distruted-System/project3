...
%% API
-export([start_link/2, init/1, handle_call/3, handle_cast/2, handle_info/2,join/2,stabilize/1,notify/2,fix_fingers/2]).
...
fix_fingers(MyIndex,1)->
  TargetIndex = MyIndex + round(math:pow(2,0)),
  Finger = find_successor(MyIndex,TargetIndex),
  gen_server:call({global, string:concat("chord", integer_to_list(MyIndex))}, {fix_fingers,Finger});
fix_fingers(MyIndex,Power)->
  TargetIndex = MyIndex + round(math:pow(2,Power-1)),
  Finger = find_successor(MyIndex,TargetIndex),
  gen_server:call({global, string:concat("chord", integer_to_list(MyIndex))}, {fix_fingers,Finger}),
  fix_fingers(MyIndex,Power-1).
...
handle_call({find_successor,ID}, _From, {SelfIndex,Power,Predecessor,Successor,FingerTable,DatList}) ->
  ...
  RealID = ID rem round(math:pow(2,Power)),
  if
    % Chord Initializing state
    SelfIndex == Successor ->
      Result = SelfIndex;
    % Normal operation
    SelfIndex < RealID and RealID < Successor ->
      Result = Successor;
    % Last node in the chord
    Successor < SelfIndex and SelfIndex < RealID  ->
      Result = Successor;
    true ->
      % Lookup the finger table
      TargetNode = closest_preceding_node(FingerTable,SelfIndex,RealID,Power),
      Result = find_successor(TargetNode,RealID)
  end,
...
handle_call({fix_fingers,Finger}, _From, {SelfIndex,Power,Predecessor,Successor,FingerTable,DatList}) ->
  {noreply, {SelfIndex,Power,Predecessor,Successor,FingerTable,[Finger]++DatList}}.

setSuccessorPre(SuccessorIndex,MyIndex)->
  gen_server:cast({global, string:concat("chord", integer_to_list(SuccessorIndex))}, {setSuccessorPre,MyIndex}).
...
handle_cast({join,TargetIndex}, {SelfIndex,Power,Predecessor,Successor,FingerTable,DatList}) ->
  ...
  setSuccessorPre(SuccessorIndex,SelfIndex),
  {noreply, {SelfIndex,Power,-1,SuccessorIndex,FingerTable,DatList}};

handle_cast({setSuccessorPre,PredecessorIndex}, {SelfIndex,Power,Predecessor,Successor,FingerTable,DatList}) ->
...
closest_preceding_node(FingerTable,SelfIndex,ID,1)->
  lists:nth(1, FingerTable);
closest_preceding_node(FingerTable,SelfIndex,ID,Power)->
  ...
  if
    SelfIndex < Finger and Finger < ID ->
      Finger;
    true ->
      closest_preceding_node(FingerTable,ID,Power-1)
  end.
